...
class MobileUNet(nn.Module):
    def __init__(self, pretrained: bool=True, num_rots: int=2) -> None:
        ...
        self.deconv1 = nn.Sequential (
            nn.Conv2d(in_channels=40, out_channels=128, kernel_size=1, stride=1),
            nn.ReLU(True),

            nn.Upsample(scale_factor=8, mode='bilinear', align_corners=True),
            
            nn.Conv2d(in_channels=128, out_channels=4, kernel_size=1, stride=1),
            nn.ReLU(True),
        )
        ...
        self.deconv3 = nn.Sequential (
            ...
            nn.Conv2d(in_channels=24, out_channels=4, kernel_size=1, stride=1),
            nn.ReLU(True),
        )
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        ############################################
        out_deconv1 = self.deconv1(x_narrow)
        #print(out_deconv1.size())
        out_deconv3 = self.deconv3(x_wide)
        #print(out_deconv3.size())
        out_deconv2 = self.deconv2(out_deconv1 + out_deconv3) 
        #print(f"size after unet stack = {out_deconv2.size()}")
        return out_deconv2
